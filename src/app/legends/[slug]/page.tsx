...
export default function LegendDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12 text-white">
      <Link href="/legends" className="text-primary hover:underline text-sm mb-8 inline-block">&larr; Back to Legends</Link>
      <div className="flex flex-col md:flex-row gap-8 items-center md:items-start bg-[#18181b] border-l-4 border-primary shadow-xl p-8">
        <Image
          src={legend.imageProfile || legend.image}
          alt={legend.name}
          width={220}
          height={220}
          className="rounded-lg border-2 border-primary bg-black object-cover"
          priority
        />
        <div className="flex-1">
          <h1 className="text-4xl font-[Duke] font-bold mb-2 tracking-tight uppercase">{legend.name}</h1>
          <div className="flex items-center gap-4 mb-4">
            <span className="px-3 py-1 bg-primary text-white rounded font-bold uppercase text-xs tracking-widest">{legend.class}</span>
          ...
          <div className="mb-4">
            ...
            <ul className="space-y-2">
              {abilities.map((ability) => (
                <li key={ability.name} className="bg-zinc-900/80 p-3 rounded border-l-4 border-primary">
