        ...
        <div className="flex-1 space-y-6">
          ...
          {legend.family && Object.values(legend.family).some((val) => val) && (
            <div className="text-sm text-zinc-300 pt-2 space-y-1 border-t border-zinc-700 mt-4">
              <h3 className="text-sm uppercase tracking-wide text-primary mb-1">Known Family</h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 text-sm">
                {legend.family.mother && (
                  <li>
                    <strong className="text-white">Mother:</strong> {legend.family.mother.name}
                    {legend.family.mother.status && ` (${legend.family.mother.status})`}
                    {legend.family.mother.notes && ` – ${legend.family.mother.notes}`}
                  </li>
                )}
                {legend.family.father && (
                  <li>
                    <strong className="text-white">Father:</strong> {legend.family.father.name}
                    {legend.family.father.status && ` (${legend.family.father.status})`}
                    {legend.family.father.notes && ` – ${legend.family.father.notes}`}
                  </li>
                )}
                {legend.family.brothers && (
                  <li>
                    <strong className="text-white">Brothers:</strong> {legend.family.brothers.name}
                    {legend.family.brothers.status && ` (${legend.family.brothers.status})`}
                    {legend.family.brothers.notes && ` – ${legend.family.brothers.notes}`}
                  </li>
                )}
              </ul>
            </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-zinc-900 p-5 border border-zinc-700 shadow">
            <h2 className="text-xl font-semibold text-primary mb-2">Voice Actor</h2>
            <p className="text-sm text-zinc-300">{legend.voice_actor}</p>
          </section>
